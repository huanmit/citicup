...
def evaluate():
    ...
    for user in users:
        cursor.execute(
            "select plogtypeid,carboncurrency from footprint where userid=%s\
            and DATE_SUB(CURDATE(), INTERVAL 7 DAY) <= %s", [user, today])
        res = cursor.fetchall()
        l1 = 0
        l2 = 0
        l3 = 0
    ...
    # d+, 这里sum里面要转置一下
    Dp = np.transpose(np.sqrt(sum(np.transpose(np.power(Rp-R, 2)))))
    # d-, 这里sum里面要转置一下
    Dn = np.transpose(np.sqrt(sum(np.transpose(np.power(Rn-R, 2)))))
    # 5.3 计算初步得分E'
    EE = Dn/(Dp+Dn)
    # 5.4 归一化
    # 这为了直观直接放到0~100
    ...
    for i in range(m):
        ...
        # print(users[i])
        cursor.execute("insert into carboncredits \
                       values(%s,%s,%s,%s,%s,%s,%s,%s)", [
                       users[i], today, round(E[i]), s, r, c, b, rk])
        cursor.execute("update user set carboncredit=%s where id=%s", [
                       round(E[i]), users[i]])
...
#X=np.array([60*np.random.rand(m,1), 80*np.random.rand(m,1),
#           1000*np.random.rand(m,1),10000*np.random.rand(m,1)])
X = np.array([48,22,957,7922,54,43,485,9594,7,76,800,6557,54,77,141,357,
             37,12,421,8491,5,77,915,9339])
X = X.reshape(m,n)
k1=-1;    # 需要正向化的列号
k2=[0,1];   # 需要进行模糊处理的列号
