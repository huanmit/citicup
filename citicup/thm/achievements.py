...
def walker(user_id: str):
    res = 0
    if bronze_walker(user_id):
        res += 1
    if silver_walker(user_id):
        res += 1
    if gold_walker(user_id):
        res += 1
    return res
...
def master_walker(user_id: str):
    res = 0
    if bronze_master_walker(user_id):
        res += 1
    if silver_master_walker(user_id):
        res += 1
    if gold_master_walker(user_id):
        res += 1
    return res
...
def bronze_walker(user_id: str):
    ...
    cursor.execute(
        "select carbonCurrency from footprint\
        where plogtypeid=1 and userid=%s", [user_id])
...
def master_rider(user_id: str):
    ...
    cursor.execute(
        "select carbonCurrency from footprint\
        where plogtypeid=2 and userid=%s", [user_id])
    results = cursor.fetchall()
    sum = 0
    for each in results:
        sum += each[0]
...
def cutleryGuardian(user_id: str):
    res = 0
    if bronze_cutleryGuardian(user_id):
        res += 1
    if silver_cutleryGuardian(user_id):
        res += 1
    if gold_cutleryGuardian(user_id):
        res += 1
    return res
...
def traveler(user_id: str):
    res = 0
    if bronze_traveler(user_id):
        res += 1
    if silver_traveler(user_id):
        res += 1
    if gold_traveler(user_id):
        res += 1
    return res
...
def master_traveler(user_id: str):
    res = 0
    if bronze_master_traveler(user_id):
        res += 1
    if silver_master_traveler(user_id):
        res += 1
    if gold_master_traveler(user_id):
        res += 1
    return res
...
def bronze_cutleryGuardian(user_id: str):
    ...
    cursor.execute(
        "select carbonCurrency from footprint\
        where plogtypeid=3 and userid=%s", [user_id])
    results = cursor.fetchall()
...
def bronze_traveler(user_id: str):
    ...
    cursor.execute(
        "select carbonCurrency from footprint\
        where plogtypeid=4 and userid=%s", [user_id])
    results = cursor.fetchall()
